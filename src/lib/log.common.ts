...
export function createCommonLogger<T extends EnhancedConsolaCommon>(
   ...
   * @private
   *
   * Provided an array of arguments, returns a new array wherein any string
   * arguments have had all instances of
   */
  const redactArgs = (args: Array<any>) => {
    return args.map(arg => (
      typeof arg === 'string' ? maskString(redactPatterns, arg) : arg)
    )
  }
  ...
  const enhancedConsola = Object.assign(createConsola({
    ...
    chalk,
    create: (childOptions: Partial<EnhancedConsolaOptions> = {}): T => {
  ...
  const logLevelPromise = Promise.resolve(level)
    .then(resolvedLevel => {
      enhancedConsola.level = resolvedLevel && Reflect.has(LogLevels, resolvedLevel)
  ...
  void (Object.keys(LogLevels) as Array<LogType>).forEach(logLevelMethodName => {
    ...
    // Extract the LogFn type here, which is not exported by Consola.
    type LogFn = typeof originalMethod

    const isDebugOrTrace = ['debug', 'trace'].includes(logLevelMethodName)

    // Decorate log method.
    Reflect.set(enhancedConsola, logLevelMethodName, (...args: Parameters<LogFn>) => {
      const doLogMessageSync = () => {
        // Since this function may be called asynchronously, we should capture
        // the current log level at the time of invocation.
